...
export class CommonService {
  ...
  checkSignOutAll(data) {
    console.log(data, "checksignoutall");
  ...
  checkAllSignOut() {
    if (!(localStorage.getItem("log") === null)) {
      //console.error("test");
      let checkData;
      //console.error("checkallsignout", checkData.user_id)
      checkData = [{
        user_id: JSON.parse(localStorage.getItem('id')),
        token: localStorage.getItem('token')
      }];
  ...
  initialLogin(pid, sessionId) {
    ...
    this.service.loginCall(loginData).subscribe(data => {
      //console.error(loginData,"initial common serv",data);
      let successData:any;
  ...
  getMnPid(mn:any, pid:any, cid:any) {
    //console.error(window.location.href.split('/')[2], "params");
    let mobileNumber:any;
    if (mn != undefined) {
      //console.error(parseInt(mn, 16).toString(10), "gfhghjhjhjh");
      mobileNumber = parseInt(mn, 16).toString(10);
      localStorage.setItem('mobileNumber', mobileNumber);
      // mobileNumber = mn.toString();
      //console.error(mobileNumber, "Converted to Decimal");
      ...
      if (window.location.href.split('/')[2] == 'zm.avvatta.com') {
        ...
        this.hexaLog(hex).subscribe(
          response => {
                    //console.error('hexalog success', response);
                    this.service.clearLoggeddevice(data).subscribe(
                      d => {
                        let resetToken = {
                          user_id: JSON.parse(JSON.stringify(d)).user_id,
                          token: JSON.parse(JSON.stringify(d)).token
                        };
                        this.service.ngResetToken(resetToken).subscribe(
                          t => {
  ...
  //     //console.error(parseInt(mn, 16).toString(10), "gfhghjhjhjh");
  //     const mobileNumber = parseInt(mn, 16).toString(10);

  //     //console.error(mobileNumber, "ddjkdjk");
  ...
  //           },
  //           error => {
  //             //console.error('Clear Logged Device Error:', error);
  //             // Handle error if needed
  //           }
  //         );
  //       },
  //       error => {
  //         //console.error('hexalog error', error);
  //         // Handle error if needed
  //       }
  //     );
  //   }
