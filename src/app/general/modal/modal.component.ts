...
export class ModalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    console.warn(window.location.href.split('/')[2]);
    ...
    else if (window.location.href.split('/')[2] == 'zm.avvatta.com') {
  ...
  loginApi(login) {
    this.service.loginCall(this.loginDta).subscribe(data => {
      ...
      console.error(successData, "login success data");
